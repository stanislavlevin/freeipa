...
skip_certbot_tests = osinfo.id not in ['fedora', 'rhel', 'altlinux']
...
def certbot_standalone_cert(host, acme_server, no_of_cert=1):
    ...
    httpd_name = host.knownservices["httpd"].systemd_name
    host.run_command(['systemctl', 'stop', httpd_name])
...
class TestACME(CALessBase):
    ...
    def test_mod_md(self):
        ...
        #
        httpd_name = self.clients[0].knownservices["httpd"].systemd_name
        self.clients[0].run_command(['systemctl', 'restart', httpd_name])
        time.sleep(15)
        ...
        #
        self.clients[0].run_command(['systemctl', 'reload', httpd_name])
        time.sleep(3)
