...
def collect_test_logs(node, logs_dict, test_config, suffix=''):
    ...
    collect_disk_usage(name, hosts, logfile_dir)
...
def collect_disk_usage(name, hosts, logfile_dir=None):
    """Collect disk usage from remote hosts

    :param name: Name under which logs are collected, e.g. name of the test
    :param hosts: List of hosts from which to collect disk usage
    :param logfile_dir: Directory to log to
    """
    if logfile_dir is None:
        return
    ...
    for host in hosts:
        logger.info("Collecting disk usage from: %s", host.hostname)
        ...
        cmd = host.run_command(
            ["df", "-h"], log_stdout=False, raiseonerr=False
        )
        if cmd.returncode:
            logger.error("An error occurred while collecting disk usage")
            continue

        with open(os.path.join(topdirname, "disk_usage"), "w") as f:
            f.write(cmd.stdout_text)
